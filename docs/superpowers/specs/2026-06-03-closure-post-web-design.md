...
## 2026-06-12 更新：發佈方式重新定義（使用者指定）

本次調整 4 點（取代下方 §3(c)、§4 對應段落的舊描述）：

1. **發布單位（新增）**：表單「發布單位」下拉改為依 `env.env` 的 `sssh_publish_unit`（如
   `系管師群組`）選取。`_select_publish_unit()`：先掃原生 `<select>` 找 option 文字含該單位者；
   fallback 自訂下拉。env 缺 key、或下拉找不到該選項 → **STOP 不發、不寫標記**（避免用錯單位）。
2. **同步顯示至（改為 `####` 驅動）**：改讀總結 `####`（校網同步顯示）行 → 以 `+` 分隔的分類清單
   （見 `summarize_doc.md` §46-47，合法詞：重要消息／處室公告／榮譽榜／課外活動／硏習資訊；空白=不選）。
   逐一點開「分類」下拉點 `最新消息-<分類>`（子字串比對 + 異體字 `硏↔研` 正規化）；
   **對不到的分類略過、能選的照選、照樣發佈**（使用者指定策略）。移除舊「研習→研習資訊／其餘→處室公告」單值邏輯。
3. **「公告於」板名不寫死**：發佈前用 `_read_board_name()` 從頁面實抓模組標題（如「圖書館公告」），寫進標記。
4. **`已公告.txt` 改三行格式**，且**只在確認發佈成功後才建立**（維持既有「標題框消失=成功」判定）：
   ```
   <ISO8601>_已公告。
   主旨:<主旨>
   公告於:<板名>。同步顯示於:<實際選到的分類,+ 串接;空則「無」>。
   ```
   「同步顯示於」記**實際點選成功**的分類（被略過者不列入,標記才誠實）。
